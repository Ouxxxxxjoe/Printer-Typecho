...
<article class="not-found">
  <h1 class="paper-title"><?php _t('页面不存在'); ?></h1>
  <p class="paper-subtitle"><?php _t('你访问的链接可能已被删除、改名，或暂时不可用。'); ?></p>

  <div class="not-found-hint">
    <?php _t('你可以尝试返回上一页、回到首页，或者搜索站内内容。'); ?>
  </div>

  <form class="not-found-search" method="get" action="<?php $this->options->siteUrl(); ?>">
    <label class="not-found-search-label" for="not-found-search-input"><?php _t('搜索'); ?></label>
    <div class="not-found-search-bar">
      <input id="not-found-search-input" type="search" name="s" placeholder="<?php _t('搜索站内内容…'); ?>" />
      <button type="submit" class="not-found-search-submit" aria-label="<?php _t('搜索'); ?>">
  ...
  <section class="paper-meta">
    <div class="meta-group">
      <p class="meta-label"><?php _t('分类'); ?></p>
      <div class="meta-tags">
