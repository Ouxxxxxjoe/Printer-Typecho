...
- AI 可读内容出口：在首页和文章页输出 Schema.org JSON-LD，并提供 `LLM.php` Markdown 索引页面模板（部署为 `/llms.html`）；配套插件 Printerllm 还可生成遵循 llmstxt.org 规范的根目录 `/llms.txt` 静态文件。
...
- `LLM.php`：AI 可读 Markdown 内容出口页面模板（通过新建独立页面渲染 `/llms.html`）。
- `functions.php`：主题入口，加载 `inc/` 子系统，定义配置项（`themeConfig`）和公共工具函数（URL 清洗、文本处理、随机文章等）。
- `inc/visit-stats.php`：访问统计子系统（建表、计数、Cookie+Session 去重、格式化）。
- `inc/llm.php`：LLM 可读内容子系统（`/llms.html` 页面渲染、文章 / 页面 / 分类 URL 构造）。
- `inc/schema.php`：Schema.org 结构化数据子系统（首页与文章页 JSON-LD）。
- `404.php`：404 页面。
- `css/style.css`：主题样式。
- `screenshot.png`：主题预览图。
- `Printerllm/`：配套插件源码目录，用于生成根目录静态 `llms.txt`，详见下方「配套插件 Printerllm」。
...
- **LLM 页面出口**：控制 `LLM.php` 页面模板是否输出机器可读 Markdown 索引（即 `/llms.html`）。
...
当前版本会在首页输出 `WebSite`、`Blog` 和 `SearchAction`，在文章页输出 `BlogPosting`。

主题的 `LLM.php` 模板用于生成 AI 可读的 Markdown 内容页面（通常部署为 `/llms.html`）：在 Typecho 后台新建一个独立页面，选择 `LLM.php` 模板，并把页面地址填入主题设置的「LLM 页面地址」即可。

根目录的 `/llms.txt`（遵循 [llmstxt.org](https://llmstxt.org) 规范的静态文件）由配套插件 **Printerllm** 生成，见下方「配套插件 Printerllm」。
...
- **LLM 页面地址**：填写 `LLM.php` 模板对应的页面地址（如 `/llms.html`），支持站内相对地址或完整 URL。填写后会像 RSS 一样在页面 `<head>` 和页脚图标中输出 `text/markdown` 发现入口。
...
- 要生成 AI 可读的 `/llms.html` 页面：在 Typecho 后台新建独立页面，选择 `LLM.php` 模板，并把页面地址填入“LLM 页面地址”。要生成根目录 `/llms.txt` 静态文件，请安装配套插件 Printerllm（见下方）。

## 配套插件 Printerllm

主题的 `LLM.php` 模板渲染的是一个**动态 Markdown 页面**（通常部署为 `/llms.html`）。而遵循 [llmstxt.org](https://llmstxt.org) 规范的根目录 `/llms.txt` 静态文件，由配套插件 **Printerllm** 在发布 / 编辑文章时自动生成。

### 主题页面 vs 插件静态文件

| | `LLM.php` 模板（主题内置） | `Printerllm` 插件 |
|---|---|---|
| **产物** | `/llms.html` 动态页面 | `/llms.txt` 静态文件（可选 `/llms-full.txt` 全文版） |
| **触发** | 用户访问页面时实时渲染 | 发布 / 编辑 / 删除文章时钩子触发，写入静态文件 |
| **规范** | 自定义 Markdown 格式 | 严格遵循 [llmstxt.org](https://llmstxt.org) |
| **关系** | 互不冲突，可同时使用 | 同上 |

### 安装

1. 把主题目录下的 `Printerllm/` 文件夹复制到 Typecho 插件目录：`/usr/plugins/Printerllm/`。
2. 登录 Typecho 后台，进入 **控制台** → **插件**。
3. 启用 **Printerllm**，进入插件设置配置文章数量等选项。
4. 启用后会在 Typecho 根目录生成 `llms.txt`；后续每次发布 / 编辑 / 删除文章会自动更新。

> 插件依赖主题 `functions.php` 中的 URL 构造与文本清洗函数（`printerPaperLlm*` / `printerPaperMarkdown*`）。即使主题未加载，插件也会自动加载并降级为自包含查询，不影响插件本身运行。详细说明见 `Printerllm/README.md`。
